...
logit.info("Reading configuration file and dataset table.")
...
output_path_dir, output_path = training_partition_path(output_dir=config.work_dir,
                                                       tomo_name=tomo_name,
                                                       fold=fold)
logit.info(f"Output directory: {output_path_dir}, Output path: {output_path}")
# print(output_path_dir)
os.makedirs(name=output_path_dir, exist_ok=True)
if os.path.isfile(output_path):
    logit.info(f"Training partition {output_path} already exists")
else:
    logit.info(f"Training partition {output_path} to be generated...")
    ...
    logit.debug(f"label_fractions_list is: {label_fractions_list}")
    ...
    logit.info(f"{selected_cubes} out of {len(label_fractions_list)} cubes in partition file.")
...
with open(snakemake_pattern, "w") as f:
    logit.info("Creating snakemake pattern")
